...
! gfortran ../modulos/tipos.f90  module_utilities.f95
...
subroutine fourier_coef(m, n, gam_1, gam_2, a, r, coef)
...
! b is the thickness of the vein
	...
	real(kind = sp)					:: f, GAMMA_r0, G_mn, bessel_value
	
	
	f = (PI*(r**2.0))/(a**2.0)
	
	G_mn = (2.0*PI/a)*sqrt(m**2.0 + n**2.0)
	
	bessel_value = bessel_j1(G_mn*r)
	
	if (m == 0 .and. n == 0) then 
		
		coef = (f*gam_1) + ((1.0-f)*gam_2)
	else
		coef = 2.0*f*(gam_1 - gam_2) * bessel_value/(G_mn*r)
